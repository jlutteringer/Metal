...
public class _Iterable {
	...
	public static <T> boolean contains(Iterable<T> iterable, T item, Equalitor<T, T> equality) {
	...
	public static <T, N> N reduce(Iterable<T> iterable, BiFunction<T, N, N> reducer) {
		return null;
	}

	public static <T, N> boolean compareElements(Iterable<? extends T> first, Iterable<? extends N> second, boolean orderMatters, Equalitor<T, N> equality) {
		if (orderMatters) {
			...
			for (T firstElement : first) {
				boolean match = false;
				for (N secondElement : second) {
					if (equality.apply(firstElement, secondElement)) {
						match = true;
					}
				}
				if (!match) {
